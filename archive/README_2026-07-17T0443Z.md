...
- **`BOOK | AGI'S MISSING LINK`** — this repository is the data-structure layer of the book *Causalontology: AGI's Missing Link*, whose thesis is that the component most conspicuously missing from today's AI is an explicit, learnable, inspectable model of cause and effect, acquired by acting on the world. The complete, lay-readable canon is right here: [`Causalontology_Standalone_Design_v18.txt`](Causalontology_Standalone_Design_v18.txt).
...
Every term above is explained for a newcomer in the master document: [`Causalontology_Standalone_Design_v18.txt`](Causalontology_Standalone_Design_v18.txt) — the complete, self-contained canon of this repository.
...
- **Durability before adoption.** Signed snapshot dumps (like Wikidata's), shipped over IPFS, BitTorrent, or HTTPS and verifiable byte-by-byte, give global redundancy before a single live peer is recruited.
- **The phased build.** (1) a persistent content-addressed node — **done**: the reference Tier A store is now durable (SQLite in WAL mode), idempotent by content address, and integrity-checked on write, with every endpoint and byte unchanged; (2) a genesis full node plus signed snapshot dumps; (3) Tier B federation with gossip and anti-entropy; (4) light clients, CDN caching, and hash-prefix sharding.

Full design: Part 21, "Commons Storage and Federation Design," in the canon ([`Causalontology_Standalone_Design_v18.txt`](Causalontology_Standalone_Design_v18.txt)).
...
```
causalontology/
  Causalontology_Standalone_Design_v18.txt   the canon (complete, lay-readable)
...
- [ ] **Next — the rest of the living network:** phases two through four of Part 21 (a genesis full node plus signed snapshot dumps, then Tier B federation with gossip and anti-entropy, then light clients and CDN caching and hash-prefix sharding)
...
> D. R. Dison. *Causalontology: AGI's Missing Link* — the Standalone Standard and the Shared Commons, Version 18 (the Persistent-Node Edition), specification 2.0.0. AI University (AIU), 2026. github.com/ai-university-aiu/causalontology
