...
> **Honest install note at 4.0.0 (updated 2026-07-26).** The first 4.0.0 publication wave is out: **Rust (crates.io), Dart (pub.dev), and Python (PyPI) are live at 4.0.0**, each verified by a fresh install from its registry passing all 137 vectors, and the fresh **`v4.0.1` tag** serves **Swift, Zig, Packagist (PHP), and the C++ source tarball** at 4.0.x, each verified fresh at the tag. The other registries below still carry the 2.0.0-era packages; each binding publishes 4.0.0 only after it passes the full 137-vector suite installed fresh from its registry, per [`docs/Causalontology_4_0_0_Release_Plan.txt`](docs/Causalontology_4_0_0_Release_Plan.txt) — as of 2026-07-22 all nineteen bindings pass the suite locally (green code); what remains is the owner's registry credentials and, for several languages, a standing human gate. The table records what is actually installable today.

| Language | Command |
|---|---|
| Python | `pip install causalontology` (**4.0.0 live on PyPI**, fresh-install verified 137/137; 1.0.0 yanked) |
| JavaScript / TypeScript | `npm install causalontology` (typings included; 2.0.0 live on npm, 1.0.0 deprecated) |
| Rust | `cargo add causalontology` (**4.0.0 live on crates.io**, fresh-install verified 137/137; 1.0.0 yanked) |
...
| Dart | `dart pub add causalontology` (**4.0.0 live on pub.dev**, fresh-install verified 137/137; 1.0.0 retracted) |
