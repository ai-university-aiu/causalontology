...
> **Honest install note at 4.0.0 (updated 2026-07-27).** **Rust (crates.io) is live at 4.0.0 and its library is proven sound** — it compiles the twenty-one JSON Schemas into the crate, so it never depended on a checkout. Note *why*, because the earlier wording here filed Rust under the wrong reason: crates.io ships only the `bindings/rust` subtree, never the repository, and Rust is safe because the schemas live inside that subtree — not because of whole-repository delivery. What is **not** sound at Rust 4.0.0 is its conformance program, which panics for any registry consumer, and its README, which told people to run it; both are fixed at 4.0.1 in this tree and unpublished. The **`v4.0.3` tag** serves **Swift, Zig, Packagist (PHP), and the C++ source tarball** at 4.0.x. Swift, PHP and the C++ tarball were sound at `v4.0.1` because a tag really does deliver the whole repository tree; **Zig was not, and was genuinely broken** — `zig fetch` prunes the tarball to the `.paths` list in the root `build.zig.zon`, `spec/` was not on it, and a consumer received ten files and zero schemas. `v4.0.3` fixes that, vendors PHP's schemas so it no longer relies on the accident, and makes the C++ CMake install carry its own copy. **Python (PyPI 4.0.0), Dart (pub.dev 4.0.0), and the Go module (`bindings/go/v4@v4.0.0`) went out defective and are being corrected**: they shipped no schemas and located them from a repository-relative path, and their conformance runners loaded repo source instead of the installed artifact, so the "fresh install 137/137" proofs measured the build machine. Installed for real, the Python wheel scored 62/137. Fixed builds are verified standalone in this tree and await the owner's named go-ahead to publish; see [PUBLISHING.md](PUBLISHING.md). Because a published version is immutable everywhere it matters, **pub.dev and the Go module are superseded at 4.0.1** — and **crates.io joins them at 4.0.1**, for its conformance binary and README rather than its library — while every channel not yet published at 4.x ships **4.0.0** correct the first time. **PyPI is the one exception**: a PyPI release can hold more than one file, so the correction goes into the existing **4.0.0** release as a second, build-tagged wheel that pip prefers, and 4.0.0 keeps its meaning there. One specification, and the 137 conformance checks are the thing that must not differ — 38 of them driven by the shared frozen vector files, per the badge note above and the measurement in [PUBLISHING.md](PUBLISHING.md). **Do not verify a published artifact against the `v4.0.0` tag**: it pins the specification-freeze commit `64b1d1a`, which predates the binding port, so `bindings/rust` there declares 2.0.0 and carries seventeen schemas. The crate on crates.io was published from `main` at `43d58a6`; PUBLISHING.md records the source commit of every live artifact. `v4.0.0`, `v4.0.1` and `v4.0.2` are left where they are and never moved, because a pushed tag is a public surface a consumer may already have pinned. The other registries below still carry the 2.0.0-era packages; each binding publishes only after the full suite passes **against the built artifact installed into a clean location, from outside the checkout**, not against the source tree beside it — that distinction is the whole of what went wrong, and the `conformance` workflow now gates both modes on every push. Per [`docs/Causalontology_4_0_0_Release_Plan.txt`](docs/Causalontology_4_0_0_Release_Plan.txt), all nineteen bindings pass the suite locally; what remains is the owner's registry credentials and, for several languages, a standing human gate. The table records what is actually installable today.
...
| Swift | `.package(url: "https://github.com/ai-university-aiu/causalontology", from: "4.0.3")` (**4.0.x via the `v4.0.3` tag**; verified 137/137 at `v4.0.1` and sound there, because a tag really does deliver the whole repository tree. Swift vendors nothing and derives its schema path from `#filePath`, so move or delete the SwiftPM checkout and you must set `CAUSALONTOLOGY_SPEC`. Swift Package Index listing submitted — [PackageList PR #14440](https://github.com/SwiftPackageIndex/PackageList/pull/14440)) |
| Go | `go get github.com/ai-university-aiu/causalontology/bindings/go/v4@v4.0.1` once published; import `.../bindings/go/v4/causalontology`. **v4.0.0 is live but defective** — a Go module ships only its own subdirectory, so `spec/schema` was never delivered and validation fails outside a checkout; **v4.0.1 embeds the schemas with `//go:embed`, retracts v4.0.0, and is verified standalone**, publication pending (the `/v2` module stays live for 2.0.x; the v1 module line is deprecated and retracted via `bindings/go/v1.0.1`) |
...
- [x] **The reference implementation gates itself — PrologAI in CI against all 119 vectors:** a new `prolog` job in [`conformance.yml`](.github/workflows/conformance.yml) checks out PrologAI beside this standard, installs SWI-Prolog, and runs PrologAI's own 119-vector harness against **this repository's canonical** [`conformance/vectors/`](conformance/vectors/) — passing only on 119/119 (V01–V119). The reference implementation is now held to the same gate as every other binding, not merely asserted in prose, and cannot silently drift from the specification. **Twenty implementations in twenty languages, twenty-two CI gates** — the additive twenty-second gate leaves the existing twenty-one untouched.
