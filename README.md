...
- **`BOOK | AGI'S MISSING LINK`** — this repository is the data-structure layer of the book *Causalontology: AGI's Missing Link*, whose thesis is that the component most conspicuously missing from today's AI is an explicit, learnable, inspectable model of cause and effect, acquired by acting on the world. The complete, lay-readable canon is right here: [`Causalontology_Standalone_Design_v16.txt`](Causalontology_Standalone_Design_v16.txt).
...
Every term above is explained for a newcomer in the master document: [`Causalontology_Standalone_Design_v16.txt`](Causalontology_Standalone_Design_v16.txt) — the complete, self-contained canon of this repository.
