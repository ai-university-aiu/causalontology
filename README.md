...
**The reference implementation is now gated, not merely asserted.** PrologAI is listed first among the bindings and named the reference implementation, so it is now held to the same standard as the rest: it is checked in Continuous Integration (CI — the practice of automatically building and testing every change) by its own `prolog` job in [`conformance.yml`](.github/workflows/conformance.yml). That job checks out PrologAI beside this standard, installs SWI-Prolog, and runs PrologAI's own 107-vector harness against **this repository's canonical** [`conformance/vectors/`](conformance/vectors/), passing only when all 107 (V01–V107) pass. Because the harness runs against the standard's own vectors, the reference implementation can no longer silently drift from the specification: it earns the label by passing the same 107 vectors as every other binding, under its own CI gate — one of the 22 — rather than by prose alone.
...
- [x] **The reference implementation gates itself — PrologAI in CI against all 107 vectors:** a new `prolog` job in [`conformance.yml`](.github/workflows/conformance.yml) checks out PrologAI beside this standard (pinned to its conformance commit `7b3aa86`), installs SWI-Prolog, and runs PrologAI's own 107-vector harness against **this repository's canonical** [`conformance/vectors/`](conformance/vectors/) — passing only on 107/107 (V01–V107). The reference implementation is now held to the same gate as every other binding, not merely asserted in prose, and cannot silently drift from the specification. **Twenty implementations in twenty languages, twenty-two CI gates** — the additive twenty-second gate leaves the existing twenty-one untouched.
