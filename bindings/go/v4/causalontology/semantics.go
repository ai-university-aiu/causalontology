...
// OrdinalUnits are the 3.0.0 ordinal (dimensionless) temporal units. A tick
// is a discrete step with NO wall-clock mapping; a tick window is ordered by
// integer comparison, and an ordinal window and a wall-clock window are
// DIFFERENT DIMENSIONS that do not compare (mixing them is never
// within-window and never overlapping).
var OrdinalUnits = map[string]bool{"ticks": true}

// dimensionOf returns "ordinal" for a tick-like unit, else "wallclock".
func dimensionOf(unit string) string {
	if OrdinalUnits[unit] {
		return "ordinal"
	}
	return "wallclock"
}

// magnitudeOf returns a comparable magnitude within ONE dimension: the raw
// tick count for an ordinal unit, seconds for a wall-clock unit. Never mix
// dimensions.
func magnitudeOf(value float64, unit string) float64 {
	if OrdinalUnits[unit] {
...
func ValidateSemantics(obj map[string]any, kind string) (bool, []string, error) {
	...
	if kind == "cross_stratal_seam" {
		if chain, present := obj["chain"]; present && chain != nil {
			if status, _ := obj["mechanism_status"].(string); status == "absent" {
				reasons = append(reasons,
	...
	if kind == "predicted_occurrence" {
		interval, _ := obj["interval"].(map[string]any)
		_, wall := interval["start"]
		_, tick := interval["start_tick"]
		if wall && tick {
			reasons = append(reasons,
...
// Admissible applies rule 4: temporal admissibility. For a wall-clock window
// elapsed is in seconds; for an ordinal ('ticks') window elapsed is a tick
// count (3.0.0). Ordering is by magnitude WITHIN the window's own dimension.
// A missing window imposes no constraint.
func Admissible(cro map[string]any, elapsed float64) (bool, error) {
	...
	lo := magnitudeOf(minimum_delay, unitName)
	hi := magnitudeOf(maximum_delay, unitName)
	return lo <= elapsed && elapsed <= hi, nil
}
...
func windowOverlap(a, b map[string]any) bool {
	...
	if dimensionOf(unitNameA) != dimensionOf(unitNameB) {
		return false
	}
	...
	loA := magnitudeOf(dminA, unitNameA)
	hiA := magnitudeOf(dmaxA, unitNameA)
	loB := magnitudeOf(dminB, unitNameB)
	hiB := magnitudeOf(dmaxB, unitNameB)
...
// fixed table. 3.0.0: an ordinal ('ticks') unit is dimensionless and has NO
// wall-clock mapping - converting one to seconds is a category error and is
// refused (a panic, the Go analog of the reference's raised exception).
func ToSeconds(duration float64, unit string) float64 {
	if OrdinalUnits[unit] {
		panic(fmt.Sprintf("'%s' is an ordinal (dimensionless) unit and has "+
...
// count; an ordinal delay and a wall-clock window (or vice versa) are
// different dimensions and never fall within one another.
func DelayWithinWindow(actualDelay, temporal map[string]any) bool {
	if len(actualDelay) == 0 || len(temporal) == 0 {
		return true
	}
	delayUnit, _ := actualDelay["unit"].(string)
	windowUnit, _ := temporal["unit"].(string)
	if dimensionOf(delayUnit) != dimensionOf(windowUnit) {
		return false // a tick delay is not within a wall-clock window
	}
	duration, _ := AsFloat(actualDelay["duration"])
	observed := magnitudeOf(duration, delayUnit)
	minDelay, _ := AsFloat(temporal["minimum_delay"])
	maxDelay, _ := AsFloat(temporal["maximum_delay"])
	lo := magnitudeOf(minDelay, windowUnit)
	hi := magnitudeOf(maxDelay, windowUnit)
...
// SeamWellformed applies 3.0.0 Rule 22 / Algorithm F: all of (a)-(g) must
// hold, else malformed_seam. A seam is a MANAGED jump across NON-ADJACENT
// strata; when it DRAWS a chain, the chain must be an adjacent-stratum path
// spanning the two endpoints' strata.
func SeamWellformed(seam map[string]any, occMap, stratumMap map[string]map[string]any) (bool, string) {
	source, _ := seam["source"].(string)
	target, _ := seam["target"].(string)
	srcS, okSrc := stratumOf(occMap, source)
	tgtS, okTgt := stratumOf(occMap, target)
	if !okSrc || !okTgt {
		return false, "malformed_seam: an endpoint has no stratum (a)"
	}
	srcScheme, _ := stratumMap[srcS]["scheme"].(string)
	tgtScheme, _ := stratumMap[tgtS]["scheme"].(string)
	if srcScheme != tgtScheme {
		return false, "malformed_seam: endpoints differ in scheme (b)"
	}
	soF, _ := AsFloat(stratumMap[srcS]["ordinal"])
	toF, _ := AsFloat(stratumMap[tgtS]["ordinal"])
	so, to := int(soF), int(toF)
	diff := so - to
	if diff < 0 {
		diff = -diff
	}
	...
	if chain, present := seam["chain"]; present && chain != nil {
		if status, _ := seam["mechanism_status"].(string); status == "absent" {
		...
		ords := []int{}
		for _, oid := range stringList(chain) {
			st, okSt := stratumOf(occMap, oid)
			if !okSt {
				return false, "malformed_seam: a chain member has no stratum (e)"
			}
			memberScheme, _ := stratumMap[st]["scheme"].(string)
			if memberScheme != srcScheme {
				return false, "malformed_seam: a chain member differs in scheme (e)"
			}
			ordF, _ := AsFloat(stratumMap[st]["ordinal"])
			ords = append(ords, int(ordF))
		}
		for _, o := range ords {
			if !(lo < o && o < hi) {
		...
		if len(ords) > 1 {
			allRising, allFalling := true, true
...
// SeamHome applies THE HOME RULE (3.0.0): a Cross Stratal Seam belongs to the
// COARSEST stratum it touches - the endpoint of the greater ordinal. Returns
// that stratum's identifier ("" when an endpoint is unstratified). A
// layer-to-stratum binding places and checks the seam by this rule.
func SeamHome(seam map[string]any, occMap, stratumMap map[string]map[string]any) string {
	source, _ := seam["source"].(string)
	target, _ := seam["target"].(string)
	srcS, okSrc := stratumOf(occMap, source)
	tgtS, okTgt := stratumOf(occMap, target)
	...
	srcOrd, _ := AsFloat(stratumMap[srcS]["ordinal"])
	tgtOrd, _ := AsFloat(stratumMap[tgtS]["ordinal"])
	if srcOrd >= tgtOrd {
		return srcS
	}
	return tgtS
...
// PredictionPairingMismatch is 4.0.0 Rule 24: true iff the prediction error's
// observed token does not instantiate the occurrent its predicted_occurrence
// instantiates (surfaces pairing_mismatch). An ABSENT observed is never a
// mismatch - it means the predicted occurrence was not fulfilled by any
// recorded occurrence. observed may be nil when no token occurrence answered
// the prediction.
func PredictionPairingMismatch(errorRec, predicted, observed map[string]any) bool {
	obs, hasObs := errorRec["observed"]
	if !hasObs || obs == nil || observed == nil {
		return false
	}
	observedInst, _ := observed["instantiates"].(string)
	predictedInst, _ := predicted["instantiates"].(string)
	return observedInst != predictedInst
}
