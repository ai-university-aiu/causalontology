...
// 2.0.0 baseline; V108-V119 are the 3.0.0 additions (the tick unit, the
...
// schemeNames are the twenty-one whole-word identifier schemes (P7): the
// seventeen 2.0.0 kinds, the 3.0.0 cross_stratal_seam, and the three 4.0.0
// kinds (attitude, predicted_occurrence, prediction_error).
...
// seamObj builds a cross_stratal_seam with its content-addressed id; a chain
// is set only when non-empty (Python's `if chain`).
func seamObj(source, target, mechanismStatus string, chain []any) map[string]any {
	o := map[string]any{"type": "cross_stratal_seam", "source": source,
		"target": target, "mechanism_status": mechanismStatus}
	if len(chain) > 0 {
		o["chain"] = chain
	}
	return mk(o)
...
// seamFixture bundles a seam with its occurrent and stratum maps.
type seamFixtureData struct {
	seam map[string]any
	omap map[string]map[string]any
	smap map[string]map[string]any
}

// makeSeamFixture mirrors Python's _seam_fixture: endpoints at the given
// stratum ordinals, plus an optional drawn chain at intervening ordinals.
func makeSeamFixture(srcOrd, tgtOrd int, mechanismStatus string, chainOrds []int) seamFixtureData {
	s := neuro()
	src := occ("source_event", idOf(s[srcOrd]))
	tgt := occ("target_event", idOf(s[tgtOrd]))
	om := map[string]map[string]any{idOf(src): src, idOf(tgt): tgt}
	sm := map[string]map[string]any{idOf(s[srcOrd]): s[srcOrd], idOf(s[tgtOrd]): s[tgtOrd]}
	var chain []any
	if chainOrds != nil {
		chain = []any{}
		for i, ord := range chainOrds {
			c := occ(fmt.Sprintf("chain_%d", i), idOf(s[ord]))
			om[idOf(c)] = c
			sm[idOf(s[ord])] = s[ord]
			chain = append(chain, idOf(c))
		}
	}
	return seamFixtureData{seamObj(idOf(src), idOf(tgt), mechanismStatus, chain), om, sm}
}

// conduitRealized mirrors Python's _conduit_realized: a fixed conduit whose
// realized_by is set only when non-empty.
func conduitRealized(realizedBy string) map[string]any {
	o := map[string]any{"type": "conduit", "label": "conn",
		"from":    "port:" + strings.Repeat("1", 64),
		"to":      "port:" + strings.Repeat("2", 64),
		"carries": L("occurrent:" + strings.Repeat("3", 64))}
	if realizedBy != "" {
...
func v108() error {
	P := cro(L(sym("occurrent:a")), L(sym("occurrent:b")), map[string]any{
		"temporal": map[string]any{"minimum_delay": 0, "maximum_delay": 5, "unit": "ticks"},
		"modality": "sufficient",
	})
	if ok, why := schemaValidExplicit(P, ""); !ok {
		return fmt.Errorf("schema: %v", why)
...
func v109() error {
	P := cro(L(sym("occurrent:a")), L(sym("occurrent:b")), map[string]any{
		"temporal": map[string]any{"minimum_delay": 2, "maximum_delay": 5, "unit": "ticks"},
	})
	adm := func(elapsed float64) bool {
		ok, err := co.Admissible(P, elapsed)
		if err != nil {
			panic(err)
		}
		return ok
	...
	if !adm(3) {
		return errors.New("3 ticks must be inside [2, 5]")
	}
	if !adm(2) || !adm(5) {
		return errors.New("the tick window is inclusive at both ends")
	}
	if adm(6) || adm(1) {
		return errors.New("ticks outside [2, 5] must not be admissible")
	}
	return nil
}

func v110() error {
	tickWindow := map[string]any{"minimum_delay": 0, "maximum_delay": 5, "unit": "ticks"}
	wallWindow := map[string]any{"minimum_delay": 0, "maximum_delay": 5, "unit": "seconds"}
	if !co.DelayWithinWindow(map[string]any{"duration": 3, "unit": "ticks"}, tickWindow) {
		return errors.New("a tick delay must fall within a tick window")
	}
	if co.DelayWithinWindow(map[string]any{"duration": 1, "unit": "ticks"}, wallWindow) {
		return errors.New("a tick delay is never within a wall-clock window")
	}
	if co.DelayWithinWindow(map[string]any{"duration": 1, "unit": "seconds"}, tickWindow) {
		return errors.New("a wall-clock delay is never within a tick window")
	}
	a := map[string]any{"causes": L(sym("occurrent:a")), "effects": L(sym("occurrent:b")),
		"temporal": tickWindow, "modality": "sufficient"}
	b := map[string]any{"causes": L(sym("occurrent:a")), "effects": L(sym("occurrent:b")),
		"temporal": wallWindow, "modality": "preventive"}
	if co.Conflicts(a, b) {
		return errors.New("disjoint dimensions must not overlap")
	}
	refused := func() (r bool) {
		defer func() {
			if recover() != nil {
		...
		co.ToSeconds(1, "ticks")
		return false
	}()
	if !refused {
		return errors.New("to_seconds accepted ticks")
	}
	return nil
}

func v111() error {
	base := map[string]any{"type": "causal_relation_object",
		"causes": L(sym("occurrent:a")), "effects": L(sym("occurrent:b")),
		"modality": "sufficient"}
	tick := co.CopyMap(base)
	tick["temporal"] = map[string]any{"minimum_delay": 0, "maximum_delay": 1, "unit": "ticks"}
	secs := co.CopyMap(base)
	secs["temporal"] = map[string]any{"minimum_delay": 0, "maximum_delay": 1, "unit": "seconds"}
	idTick, err := co.Identify(tick, "")
	if err != nil {
		return err
	}
	idSecs, err := co.Identify(secs, "")
	if err != nil {
		return err
	}
	if idTick == idSecs {
		return errors.New("the unit must be identity-bearing")
	}
	const pinned = "causal_relation_object:" +
		"d8daf899daa3ee03caa6b1425cc6d4d33cef20d951e1203ffd35df29857aa43c"
	if idSecs != pinned {
		return fmt.Errorf("the pinned 2.0.0 identifier must hold: got %s", idSecs)
	}
	return nil
}
...
func v112() error {
	f := makeSeamFixture(14, 4, "unmodeled", nil)
	if ok, why := schemaValidExplicit(f.seam, ""); !ok {
		return fmt.Errorf("schema: %v", why)
	}
	ok, why, err := co.ValidateSemantics(f.seam, "")
	if err != nil {
		return err
	}
	if !ok {
	...
	if ok, why := co.SeamWellformed(f.seam, f.omap, f.smap); !ok {
		return fmt.Errorf("seam_wellformed: %s", why)
	}
	return nil
}

func v113() error {
	a := makeSeamFixture(14, 4, "unmodeled", nil)
	b := makeSeamFixture(14, 4, "absent", nil)
	if ok, why := schemaValidExplicit(b.seam, ""); !ok {
		return fmt.Errorf("schema: %v", why)
	}
	if ok, why := co.SeamWellformed(b.seam, b.omap, b.smap); !ok {
		return fmt.Errorf("seam_wellformed: %s", why)
	}
	if idOf(a.seam) == idOf(b.seam) {
		return errors.New("mechanism_status must be identity-bearing")
	}
	return nil
}

func v114() error {
	drawn := makeSeamFixture(14, 4, "unmodeled", []int{9, 7, 6, 5})
	if ok, why := schemaValidExplicit(drawn.seam, ""); !ok {
		return fmt.Errorf("schema: %v", why)
	}
	if ok, why := co.SeamWellformed(drawn.seam, drawn.omap, drawn.smap); !ok {
		return fmt.Errorf("seam_wellformed: %s", why)
	}
	bad := makeSeamFixture(14, 4, "absent", []int{9, 7, 6, 5})
	ok, why, err := co.ValidateSemantics(bad.seam, "")
	if err != nil {
		return err
	}
	if ok || !mentions(why, "contradictory_seam") {
		return fmt.Errorf("semantics must reject the drawn 'absent' seam: %v", why)
	}
	if ok2, _ := co.SeamWellformed(bad.seam, bad.omap, bad.smap); ok2 {
		return errors.New("the drawn 'absent' seam must be malformed")
	}
	return nil
}
...
func v115() error {
	f := makeSeamFixture(14, 4, "unmodeled", nil)
	s := neuro()
	if home := co.SeamHome(f.seam, f.omap, f.smap); home != idOf(s[14]) {
		return errors.New("home must be the coarsest (max ordinal) stratum")
	}
	return nil
}

func v116() error {
	adj := makeSeamFixture(6, 5, "unmodeled", nil) // adjacent (gap 1)
	if ok, _ := co.SeamWellformed(adj.seam, adj.omap, adj.smap); ok {
		return errors.New("adjacent endpoints must be malformed")
	}
	costratal := makeSeamFixture(6, 6, "unmodeled", nil) // co-stratal (gap 0)
	if ok, _ := co.SeamWellformed(costratal.seam, costratal.omap, costratal.smap); ok {
		return errors.New("co-stratal endpoints must be malformed")
	}
	f := makeSeamFixture(14, 4, "unmodeled", nil)
	if !strings.HasPrefix(idOf(f.seam), "cross_stratal_seam:") {
		return errors.New("a seam must mint in the new identity scheme")
	}
	return nil
}
...
func v117() error {
	c := conduitRealized("causal_relation_object:" + strings.Repeat("a", 64))
	if ok, why := schemaValidExplicit(c, ""); !ok {
		return fmt.Errorf("schema: %v", why)
	}
	c2 := conduitRealized("native:region_stratum_predict")
	if ok, why := schemaValidExplicit(c2, ""); !ok {
		return fmt.Errorf("schema: %v", why) // a native scheme reference is legal
	}
	return nil
}
...
func v118() error {
	...
	if idOf(bound) == idOf(unbound) {
		return errors.New("realized_by must be identity-bearing")
	}
	const pinned = "conduit:" +
		"dc4af3b1a24f0560d5ebcee488779f06ab3c78301cfb9d0c7edff80bc62e27a6"
	if idOf(unbound) != pinned {
		return fmt.Errorf("the pinned 2.0.0 identifier must hold: got %s", idOf(unbound))
	}
	return nil
}
...
func v119() error {
	unbound := conduitRealized("")
	if ok, why := schemaValidExplicit(unbound, ""); !ok {
		return fmt.Errorf("schema: %v", why) // unbound is legal
	}
	bad := co.CopyMap(unbound)
	bad["realized_by"] = "not-a-scheme-qualified-reference"
	if ok, _ := schemaValidExplicit(bad, "conduit"); ok {
		return errors.New("a malformed realized_by reference must be rejected")
	}
	return nil
}
...
// attitudeObj builds an attitude with its content-addressed id.
func attitudeObj(holder, attitudeType, content string) map[string]any {
	return mk(map[string]any{"type": "attitude", "holder": holder,
		"attitude_type": attitudeType, "content": content})
}

// predictedObj builds a predicted_occurrence; strength is set only when
// present.
func predictedObj(instantiates string, interval map[string]any, predictor string, strength *float64) map[string]any {
	o := map[string]any{"type": "predicted_occurrence", "instantiates": instantiates,
...
// predictionErrorObj builds a prediction_error; observed is set only when
// present.
func predictionErrorObj(predictedID string, discrepancy float64, observed string) map[string]any {
	o := map[string]any{"type": "prediction_error", "predicted": predictedID,
...
// predictorID mirrors Python's _predictor: a modeled agent, never a key.
func predictorID() string {
	c := cnt("forecasting_mind")
	return idOf(individual(idOf(c), "predictor_p", ""))
}

// believerID mirrors Python's _believer.
func believerID(designator string) string {
	c := cnt("believing_mind")
	return idOf(individual(idOf(c), designator, ""))
}

// -- Group X: prediction and prediction error --
func v120() error {
	o := occ("rainfall_begins", "")
	p := predictedObj(idOf(o), map[string]any{"start_tick": 3, "end_tick": 8}, predictorID(), nil)
	if ok, why := schemaValidExplicit(p, ""); !ok {
		return fmt.Errorf("schema: %v", why)
	}
	ok, why, err := co.ValidateSemantics(p, "")
	...
	if !strings.HasPrefix(idOf(p), "predicted_occurrence:") {
		return errors.New("a forecast must mint in the new identity scheme")
	}
	report, err := co.Identify(map[string]any{"type": "token_occurrence",
		"instantiates": idOf(o),
		"interval":     map[string]any{"start_tick": 3, "end_tick": 8}}, "token_occurrence")
	if err != nil {
		return err
	}
	if idOf(p) == report {
		return errors.New("a forecast is not a report")
	}
	if !strings.HasPrefix(report, "token_occurrence:") {
		return errors.New("the report keeps its own scheme")
	}
	return nil
}
...
func v121() error {
	o := occ("rainfall_begins", "")
	wall := map[string]any{"start": "2026-07-23T00:00:00Z", "end": "2026-07-24T00:00:00Z"}
	who := predictorID()
	withStrength := predictedObj(idOf(o), wall, who, floatPtr(0.8))
	without := predictedObj(idOf(o), wall, who, nil)
	for _, p := range []map[string]any{withStrength, without} {
		if ok, why := schemaValidExplicit(p, ""); !ok {
			return fmt.Errorf("schema: %v", why)
		}
		ok, why, err := co.ValidateSemantics(p, "")
	...
	if idOf(withStrength) == idOf(without) {
		return errors.New("strength must be identity-bearing")
	}
	return nil
}
...
func v122() error {
	o := occ("rainfall_begins", "")
	bad := mk(map[string]any{"type": "predicted_occurrence", "instantiates": idOf(o),
		"interval": map[string]any{"start_tick": 3}})
	ok, why := schemaValidExplicit(bad, "predicted_occurrence")
	if ok || !mentions(why, "predictor") {
		return fmt.Errorf("expected predictor error: %v", why)
	}
	return nil
}
...
func v123() error {
	o := occ("rainfall_begins", "")
	both := predictedObj(idOf(o),
		map[string]any{"start": "2026-07-23T00:00:00Z", "start_tick": 3}, predictorID(), nil)
	if ok, why := schemaValidExplicit(both, ""); !ok {
		return fmt.Errorf("schema: %v", why)
	}
	ok, why, err := co.ValidateSemantics(both, "")
	if err != nil {
		return err
	}
	if ok || !mentions(why, "dimension_conflict") {
		return fmt.Errorf("semantics must reject both dimensions: %v", why)
	}
	return nil
}
...
func v124() error {
	o := occ("rainfall_begins", "")
	p := predictedObj(idOf(o), map[string]any{"start": "2026-07-23T00:00:00Z"}, predictorID(), nil)
	t := token(idOf(o), map[string]any{"start": "2026-07-23T06:00:00Z"}, nil, "")
	err := predictionErrorObj(idOf(p), 0.0, idOf(t))
	if ok, why := schemaValidExplicit(err, ""); !ok {
		return fmt.Errorf("schema: %v", why)
	}
	ok, why, verr := co.ValidateSemantics(err, "")
	if verr != nil {
		return verr
	}
	if !ok {
	...
	if co.PredictionPairingMismatch(err, p, t) {
		return errors.New("a matching observation is not a mismatch")
	}
	return nil
}
...
func v125() error {
	o := occ("rainfall_begins", "")
	p := predictedObj(idOf(o), map[string]any{"start": "2026-07-23T00:00:00Z"}, predictorID(), nil)
	err := predictionErrorObj(idOf(p), -1.0, "")
	if ok, why := schemaValidExplicit(err, ""); !ok {
		return fmt.Errorf("schema: %v", why)
	}
	ok, why, verr := co.ValidateSemantics(err, "")
	...
	if _, has := err["observed"]; has {
		return errors.New("observed must be absent")
	}
	if co.PredictionPairingMismatch(err, p, nil) {
		return errors.New("an unfulfilled prediction is not a mismatch")
	}
	return nil
}
...
func v126() error {
	o := occ("rainfall_begins", "")
	p := predictedObj(idOf(o), map[string]any{"start_tick": 0}, predictorID(), nil)
	bad := mk(map[string]any{"type": "prediction_error", "predicted": idOf(p)})
	ok, why := schemaValidExplicit(bad, "prediction_error")
	if ok || !mentions(why, "discrepancy") {
		return fmt.Errorf("expected discrepancy error: %v", why)
	}
	return nil
}
...
func v127() error {
	...
	p := predictedObj(idOf(o), map[string]any{"start": "2026-07-23T00:00:00Z"}, predictorID(), nil)
	t := token(idOf(other), map[string]any{"start": "2026-07-23T06:00:00Z"}, nil, "")
	err := predictionErrorObj(idOf(p), 1.0, idOf(t))
	if ok, why := schemaValidExplicit(err, ""); !ok {
		return fmt.Errorf("schema: %v", why)
	}
	if !co.PredictionPairingMismatch(err, p, t) {
		return errors.New("must surface a pairing mismatch")
	}
	return nil
}

// -- Group Y: attitude and theory of mind --
func v128() error {
	st, _ := stateFixture("quantity", map[string]any{"quantity": 15.0, "unit": "ug/dL"}, "ug/dL")
	att := attitudeObj(believerID("holder_h"), "believes", idOf(st))
	if ok, why := schemaValidExplicit(att, ""); !ok {
		return fmt.Errorf("schema: %v", why)
	}
	ok, why, err := co.ValidateSemantics(att, "")
	if err != nil {
		return err
	}
	if !ok {
...
func v129() error {
	...
	b := occ("light_on", "")
	actual := cro(L(idOf(a)), L(idOf(b)), map[string]any{"modality": "sufficient"})
	believed := cro(L(idOf(a)), L(idOf(b)), map[string]any{"modality": "preventive"})
	if !co.Conflicts(believed, actual) {
		return errors.New("the CLAIMS must contradict")
	}
	att := attitudeObj(believerID("holder_h"), "believes", idOf(believed))
	if ok, why := schemaValidExplicit(att, ""); !ok {
		return fmt.Errorf("schema: %v", why)
	}
	ok, why, err := co.ValidateSemantics(att, "")
	...
	if !ok {
		return fmt.Errorf("semantics: %v", why) // validity unaffected
	}
	s := co.NewStore(true)
	for _, obj := range []map[string]any{a, b, actual, att} {
		if _, err := s.Put(obj, ""); err != nil {
			return err
		}
	}
	if len(s.Gaps("conflict")) != 0 {
		return errors.New("Rule 25: NO conflict must be raised")
	}
	return nil
}
...
func v130() error {
	o := occ("rainfall_begins", "")
	att := attitudeObj(believerID("holder_h"), "desires", idOf(o))
	if ok, why := schemaValidExplicit(att, ""); !ok {
		return fmt.Errorf("schema: %v", why)
	}
	ok, why, err := co.ValidateSemantics(att, "")
...
func v131() error {
	o := occ("press_button", "")
	att := attitudeObj(believerID("holder_h"), "intends", idOf(o))
	if ok, why := schemaValidExplicit(att, ""); !ok {
		return fmt.Errorf("schema: %v", why)
	}
	ok, why, err := co.ValidateSemantics(att, "")
...
func v132() error {
	st, _ := stateFixture("boolean", map[string]any{"boolean": true}, "")
	inner := attitudeObj(believerID("holder_b"), "believes", idOf(st))
	outer := attitudeObj(believerID("holder_a"), "believes", idOf(inner))
	for _, att := range []map[string]any{inner, outer} {
		if ok, why := schemaValidExplicit(att, ""); !ok {
			return fmt.Errorf("schema: %v", why)
		}
		ok, why, err := co.ValidateSemantics(att, "")
	...
	if idOf(outer) == idOf(inner) {
		return errors.New("ids must differ")
	}
	if content, _ := outer["content"].(string); content != idOf(inner) {
		return errors.New("the outer content must be the inner attitude")
	}
	return nil
}
...
func v133() error {
	o := occ("rainfall_begins", "")
	bad := mk(map[string]any{"type": "attitude", "holder": believerID("holder_h"),
		"attitude_type": "suspects", "content": idOf(o)})
	ok, why := schemaValidExplicit(bad, "attitude")
	if ok || !mentions(why, "attitude_type") {
		return fmt.Errorf("expected attitude_type error: %v", why)
	}
	return nil
}
...
func v134() error {
	o := occ("rainfall_begins", "")
	bad := mk(map[string]any{"type": "attitude", "holder": believerID("holder_h"),
		"attitude_type": "believes", "content": idOf(o), "strength": 0.9})
	ok, why := schemaValidExplicit(bad, "attitude")
	if ok || !mentions(why, "strength") {
		return fmt.Errorf("expected strength error: %v", why)
	}
	return nil
}
...
func v135() error {
	o := occ("rainfall_begins", "")
	att := attitudeObj(believerID("holder_h"), "expects", idOf(o))
	a, err := signed("assertion", map[string]any{"about": idOf(att),
		"evidence_type": "observation", "confidence": 0.9}, "signer", 0)
	if err != nil {
		return err
	}
	if ok, why := schemaValidExplicit(a, ""); !ok {
	...
	if !co.VerifyRecord(a, "") {
		return errors.New("the assertion must verify")
	}
	holder, _ := att["holder"].(string)
	holderScheme, _, _ := strings.Cut(holder, ":")
	if holderScheme != "token_individual" {
		return errors.New("the holder must be a modeled agent")
	}
	source, _ := a["source"].(string)
	sourceScheme, _, _ := strings.Cut(source, ":")
	if sourceScheme != "ed25519" {
		return errors.New("the source must be a signing key")
	}
	if holder == source {
		return errors.New("holder and source are different things")
	}
	return nil
}
...
func v136() error {
	...
	secs := map[string]any{"type": "causal_relation_object",
		"causes": L(sym("occurrent:a")), "effects": L(sym("occurrent:b")),
		"modality": "sufficient",
		"temporal": map[string]any{"minimum_delay": 0, "maximum_delay": 1, "unit": "seconds"}}
	idSecs, err := co.Identify(secs, "")
	...
	if idSecs != "causal_relation_object:"+
		"d8daf899daa3ee03caa6b1425cc6d4d33cef20d951e1203ffd35df29857aa43c" {
		return errors.New("the 3.0.0 wall-clock identifier must hold under 4.0.0")
	}
	// the V118 unbound conduit, re-pinned under 4.0.0
	unbound := conduitRealized("")
	if idOf(unbound) != "conduit:"+
		"dc4af3b1a24f0560d5ebcee488779f06ab3c78301cfb9d0c7edff80bc62e27a6" {
		return errors.New("the 3.0.0 unbound-conduit identifier must hold under 4.0.0")
	}
	return nil
}
...
func v137() error {
	hexid := strings.Repeat("0", 64)
	// The abbreviated prefixes are the deliberate negative tests; each is
	// assembled so it survives any whole-word re-mint pass.
	attAbbr := "a" + "t" + "t"
	prdAbbr := "p" + "r" + "d"
	errAbbr := "e" + "r" + "r"
	badAtt := map[string]any{"type": "attitude", "id": attAbbr + ":" + hexid,
		"holder": "token_individual:" + hexid, "attitude_type": "believes",
		"content": "state_assertion:" + hexid}
	if ok, _ := schemaValidExplicit(badAtt, "attitude"); ok {
		return errors.New("abbreviated attitude scheme must be rejected")
	}
	badPrd := map[string]any{"type": "predicted_occurrence", "id": prdAbbr + ":" + hexid,
		"instantiates": "occurrent:" + hexid, "interval": map[string]any{"start_tick": 0},
		"predictor": "token_individual:" + hexid}
	if ok, _ := schemaValidExplicit(badPrd, "predicted_occurrence"); ok {
		return errors.New("abbreviated predicted_occurrence scheme must be rejected")
	}
	badErr := map[string]any{"type": "prediction_error", "id": errAbbr + ":" + hexid,
		"predicted": "predicted_occurrence:" + hexid, "discrepancy": 0.0}
	if ok, _ := schemaValidExplicit(badErr, "prediction_error"); ok {
		return errors.New("abbreviated prediction_error scheme must be rejected")
	}
	wholeAtt := co.CopyMap(badAtt)
	wholeAtt["id"] = "attitude:" + hexid
	if ok, why := schemaValidExplicit(wholeAtt, "attitude"); !ok {
		return fmt.Errorf("whole-word attitude must validate: %v", why)
	}
	wholePrd := co.CopyMap(badPrd)
	wholePrd["id"] = "predicted_occurrence:" + hexid
	if ok, why := schemaValidExplicit(wholePrd, "predicted_occurrence"); !ok {
		return fmt.Errorf("whole-word predicted_occurrence must validate: %v", why)
	}
	wholeErr := co.CopyMap(badErr)
	wholeErr["id"] = "prediction_error:" + hexid
	if ok, why := schemaValidExplicit(wholeErr, "prediction_error"); !ok {
		return fmt.Errorf("whole-word prediction_error must validate: %v", why)
	}
	return nil
}
