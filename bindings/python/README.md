...
causalontology-py is CONFORMANT to the suite (pre-freeze, symbolic-id normalization).
```
...
names become real Ed25519 keypairs seeded from the name), so every normative
behavior is exercised with well-formed data and real signatures. The 1.0.0
freeze pins concrete bytes into the vectors themselves.
