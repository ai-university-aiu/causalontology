...
| [causalontology-zig](zig/) | **live by git tag** — pin the tag tarball by url+hash (`zig fetch` prints it); subdirectory package, so reference `dep.path("bindings/zig/src/causalontology.zig")` (a repo-root manifest for the clean `dep.module()` flow is not yet shipped) | **verified locally — 107/107**; std-lib crypto (Zig 0.13.0), insertion-ordered maps throughout |
