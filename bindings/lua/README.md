...
| `causalontology/schema.lua` | validation against the twenty-one JSON Schemas in `spec/schema/` (a small interpreter for exactly the keywords those schemas use); Lua has no regex engine, so a dedicated, regex-free matcher recognizes the schemas' anchored pattern families (fixed-length hex, prefixed 64-hex identifiers, prefix alternations, snake_case labels, and the scheme-qualified `realized_by` reference) and refuses anything else |
...
137/137, the bignum layer is cross-checked against Python big-integer
...
Packaging: `causalontology-4.0.0-1.rockspec` (LuaRocks, zero
dependencies); the `causalontology-2.0.0-1.rockspec` stays for history
(LuaRocks has no yank).
