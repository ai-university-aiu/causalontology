...
|---|---|
| `Causalontology/Json.cs` | a lossless JSON layer: a small recursive-descent parser whose numbers keep their source literal (no `.`, `e`, or `E` → `long`; otherwise `double`), and a `JsonMap` object type with an explicit key insertion-order list — we do not rely on `Dictionary`'s de-facto ordering |
