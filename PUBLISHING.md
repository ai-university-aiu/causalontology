...
| PyPI | python | 4.0.0 | **no** | **4.0.0**, as a second file — `causalontology-4.0.0-1-py3-none-any.whl` | PyPI burns a *filename* forever, but a *release* can still take another file. The corrected wheel carries wheel build tag `1`; the build tag is the last element of pip's candidate sort key, so it beats the untagged wheel on every tie. The two broken files are deleted first, so 4.0.0 then serves the fix and nothing else. Three permanent consequences, accepted deliberately: 4.0.0 can never carry an sdist again (one sdist per release, and that filename is burned), hash-pinned consumers get a loud mismatch instead of a silent broken install, and the window shuts on **2026-08-09** because PyPI refuses new files on releases older than 14 days |
...
|---|---|---|---|
| PyPI | 4.0.0 | ships no schemas; truly-installed wheel scores **62/137** — every schema-validation vector fails with file-not-found | **4.0.0 build 1** built as `causalontology-4.0.0-1-py3-none-any.whl`, 32 entries carrying all 21 schemas, `Build: 1` in its `WHEEL` metadata; installed into a clean virtual environment, `CAUSALONTOLOGY_SPEC` unset, run from outside the checkout — **137/137**, with `pip show` reporting `Version: 4.0.0`. Build it only with the build number (see the comment in `pyproject.toml`); a plain build emits the burned filename and PyPI will reject it |
